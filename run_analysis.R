...
# binds the test files into one data frame
test <- cbind(test_subject, test_y, test_x)

# binds the train files into one data frame
train <- cbind(train_subject, train_y, train_x)

# binds the test and train data frames into one data frame
...
# changes remaining column names using features   
colnames(data)[3:563] = as.character(features[, 2])

# gets vector of numbers of column with either subject or activity in its name
columns_subject_activity <- grep("subject|activity", names(data))

# gets vector of numbers of columns with either mean or std in its name 
...
headers = c("id", "activity_name")
colnames(activities)[1:2] = headers
...
# removing parantheses in colnames
